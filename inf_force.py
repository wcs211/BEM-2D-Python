import numpy as np
from normal_vectors import panel_vectors
...
# Others: xp1, xp2, zp, nx, nz, tx, tz, xp1_e, xp2_e, zp_e
    ...
    if i > 0:
        ...
        (xp1,xp2,zp) = transformation(Body.x_col,Body.z_col,Body.x,Body.z)
        ...
        (xp1,xp2,zp) = transformation(Body.x_col,Body.z_col,Edge.x,Edge.z)
        
        if i > 1: # No wake panels until i==2
            # Wake doublets influencing the body
            (xp1_w,xp2_w,zp_w) = transformation(Body.x_col,Body.z_col,Wake.x[:i],Wake.z[:i])
